import type { SupportTicketStatus, SupportTicketPriority, SupportTicketCategory, ContactRole, ActivityType, TaskStatus } from '@/drizzle/support-schema'
...
export type KnowledgeBaseArticle = InferSelectModel<typeof knowledgeBaseArticles>
...
export async function getTicketComments(ticketId: string): Promise<Array<TicketComment & { userName?: string | null }>> {
  const db = getDb()
...
export async function getTicketStats(schoolId?: string): Promise<TicketStats> {
  ...
  const baseConditions = schoolId ? [eq(supportTickets.schoolId, schoolId)] : []
  ...
  const [satisfactionResult] = await db
    ...
    .from(supportTickets)
    .where(and(isNotNull(supportTickets.satisfactionRating), ...baseConditions))
...
export async function getRecentTickets(limit = 5): Promise<TicketWithDetails[]> {
...
export async function getKnowledgeBaseArticles(
  ...
  const articles = await db
    ...
    .from(knowledgeBaseArticles)
    .where(conditions.length > 0 ? and(...conditions) : undefined)
    .orderBy(desc(knowledgeBaseArticles.views), desc(knowledgeBaseArticles.updatedAt))
    .limit(limit)
    .offset(offset)
...
export async function getArticleById(id: string): Promise<KnowledgeBaseArticle | null> {
  const db = getDb()
...
export async function getFeaturedArticles(limit = 5): Promise<KnowledgeBaseArticle[]> {
  ...
  const articles = await db
    ...
    .orderBy(desc(knowledgeBaseArticles.isFeatured), desc(knowledgeBaseArticles.views))
    .limit(limit)
...
export async function getCrmContacts(
  ...
  limit = 50,
  offset = 0,
): Promise<{ contacts: (CrmContact & { schoolName?: string | null })[]; total: number }> {
  const db = getDb()
...
export async function getContactById(id: string): Promise<(CrmContact & { schoolName?: string | null; activities: CrmActivity[] }) | null> {
  const db = getDb()
...
export async function getCrmTasks(
  ...
  limit = 50,
  offset = 0,
): Promise<{ tasks: (CrmTask & { contactName?: string | null; schoolName?: string | null })[]; total: number }> {
  const db = getDb()
...
export function getTicketPriorityCounts(): Record<SupportTicketPriority, number> {
  return {
    low: 0,
    medium: 0,
    high: 0,
