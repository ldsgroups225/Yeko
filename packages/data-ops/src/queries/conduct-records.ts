...
export async function updateConductRecord(
  id: string,
...
export async function markConductParentAcknowledged(
  id: string,
  response?: string,
) {
  const db = getDb();
...
export async function getStudentConductSummary(
  studentId: string,
  schoolYearId: string,
) {
  const db = getDb();
  ...
  const [summary] = await db
    .select({
      ...
      behaviorCount: sql<number>`COUNT(*) FILTER (WHERE ${conductRecords.category} = 'behavior')`,
      academicCount: sql<number>`COUNT(*) FILTER (WHERE ${conductRecords.category} = 'academic')`,
