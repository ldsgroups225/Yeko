...
import type { YekoLogContext, UserRole } from "../types";
...
export function normalizeContext(context?: YekoLogContext): YekoLogContext | undefined {
  ...
  if (context.schoolId) normalized.schoolId = context.schoolId;
  if (context.organizationId) normalized.organizationId = context.organizationId;
  if (context.userId) normalized.userId = context.userId;
  if (context.userRole) normalized.userRole = context.userRole;
  if (context.sessionId) normalized.sessionId = context.sessionId;
  if (context.academicYearId) normalized.academicYearId = context.academicYearId;
  if (context.semesterId) normalized.semesterId = context.semesterId;
  if (context.courseId) normalized.courseId = context.courseId;
  if (context.subjectId) normalized.subjectId = context.subjectId;
  if (context.requestId) normalized.requestId = context.requestId;
  if (context.ip) normalized.ip = context.ip;
  if (context.userAgent) normalized.userAgent = context.userAgent;
  if (typeof context.duration === "number") normalized.duration = context.duration;
  if (typeof context.memoryUsage === "number") normalized.memoryUsage = context.memoryUsage;
