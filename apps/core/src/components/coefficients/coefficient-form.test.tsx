...
function CoefficientForm({
  ...
  const [selectedCoefficients, setSelectedCoefficients] = React.useState<string[]>([])
  const [editingCoefficient, setEditingCoefficient] = React.useState<string | null>(null)
  ...
  const handleEditCoefficient = (coefficientId: string, currentValue: number) => {
  ...
  const handleSaveCoefficient = (coefficientId: string) => {
    ...
    if (value === 0) {
      ...
      if (!window.confirm('Un poids de zéro peut affecter les calculs. Continuer?')) {
        return
      }
    }
            ...
            <table data-testid="coefficients-table">
                ...
                <tr>
                  {bulkEditMode && (
                    <th data-testid="select-all-header">
                      <input
                        type="checkbox"
                        checked={selectedCoefficients.length === coefficients.length}
              ...
              <tbody>
                {coefficients.map(coefficient => (
                  <tr key={coefficient.id} data-testid={`coefficient-row-${coefficient.id}`}>
                    {bulkEditMode && (
                      <td>
                        <input
                          type="checkbox"
                      ...
                      {editingCoefficient === coefficient.id
                        ? (
                            <input
                              ...
                              onChange={e => setFormData({ ...formData, [coefficient.id]: e.target.value })}
                            ...
                            <span
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter' || e.key === ' ') {
                                  handleEditCoefficient(coefficient.id, coefficient.weight)
                                }
                              }}
                            >
                              {coefficient.weight}
                    ...
                    <td>
                      <button
                        type="button"
                        onClick={() => onSubmit({ action: 'delete', id: coefficient.id })}
...
describe('coefficient Form Component', () => {
  ...
  describe('rendering Tests', () => {
    test('should render empty state when no coefficients', () => {
      ...
      expect(screen.getByText('Aucun coefficient configuré')).toBeInTheDocument()
      expect(screen.getByTestId('empty-add-btn')).toBeInTheDocument()
      expect(screen.queryByTestId('coefficients-table')).not.toBeInTheDocument()
    })
    ...
    test('should render coefficients table when data exists', () => {
      ...
      expect(screen.getByTestId('weight-display-coeff-1')).toHaveTextContent('3')
      expect(screen.getByTestId('weight-display-coeff-2')).toHaveTextContent('2')
    })
    ...
    test('should render without series when not applicable', () => {
      const coeffWithoutSeries = [{
        ...
        grade: mockCoefficients[0]?.grade || { name: 'Test Grade', code: 'TG' },
        subject: mockCoefficients[0]?.subject || { name: 'Test Subject', shortName: 'TS' },
        series: undefined,
      }]
  ...
  describe('inline Edit Mode Tests', () => {
    test('should enter edit mode when weight is clicked', async () => {
      ...
      expect(screen.queryByTestId('weight-display-coeff-1')).not.toBeInTheDocument()
    })
    ...
    test('should save coefficient on blur with valid value', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('weight-display-coeff-1')).toBeInTheDocument()
      })
      expect(screen.getByTestId('weight-display-coeff-1')).toHaveTextContent('3') // Original value in mock
      expect(screen.queryByTestId('weight-input-coeff-1')).not.toBeInTheDocument()
    })
    ...
    test('should cancel edit on Escape key', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('weight-display-coeff-1')).toHaveTextContent('3') // Original value
      })
    })
  })
  ...
  describe('bulk Operations Tests', () => {
    ...
    test('should select all coefficients', async () => {
      ...
      expect(screen.getByTestId('bulk-update-btn')).toHaveTextContent('Mettre à Jour (2)')
    })
    ...
    test('should select individual coefficients', async () => {
      ...
      expect(screen.getByTestId('bulk-update-btn')).toHaveTextContent('Mettre à Jour (1)')
  ...
  describe('form State and Navigation Tests', () => {
    ...
    test('should show loading state during submission', () => {
      ...
      expect(screen.getByTestId('save-button')).toHaveTextContent('Enregistrement...')
    })
  })
