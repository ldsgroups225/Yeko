                ...
                >
                  ...
                  <TableCell className="py-2">
                    <div className="flex items-center justify-end gap-1">
                      {(Object.entries(STATUS_CONFIG) as [StudentAttendanceStatus, typeof STATUS_CONFIG['present']][]).map(([status, config]) => (
                        <StatusButton
                          key={status}
...
function StatusButton({ active, onClick, config }: { active: boolean, onClick: () => void, config: typeof STATUS_CONFIG[StudentAttendanceStatus] }) {
