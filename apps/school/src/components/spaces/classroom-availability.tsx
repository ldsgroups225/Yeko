...
function StatsCards({ available, occupied, maintenance, inactive }: { available: number, occupied: number, maintenance: number, inactive: number }) {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-4" role="list" aria-label="Statistiques des salles">
      {stats.map((stat, index) => (
        ...
        >
          <Card role="listitem" className={`rounded-3xl border bg-card/40 backdrop-blur-xl shadow-sm hover:shadow-md transition-all duration-300 ${stat.bgColor.replace('bg-', 'border-').replace('/10', '/20')}`}>
            ...
            <CardContent>
              <div className="flex flex-col gap-1">
                <span className="text-3xl font-black tracking-tight" aria-label={`${stat.value} ${stat.title}`}>
                  {stat.value}
                </span>
                <span className="text-xs font-medium text-muted-foreground/80">{stat.description}</span>
              </div>
            </CardContent>
          </Card>
        </motion.div>
...
export function ClassroomAvailability() {
  ...
  const availableCount = classroomList.filter(c => c.assignedClassesCount === 0 && c.classroom.status === 'active').length
  const occupiedCount = classroomList.filter(c => c.assignedClassesCount > 0).length
  const maintenanceCount = classroomList.filter(c => c.classroom.status === 'maintenance').length
  const inactiveCount = classroomList.filter(c => c.classroom.status === 'inactive').length

  return (
    <div className="space-y-8" role="region" aria-label="Disponibilité des salles de classe">
      <StatsCards
          ...
          <Table>
            <TableHeader className="bg-muted/50">
              <TableRow className="hover:bg-transparent border-border/40">
                <TableHead className="font-semibold text-muted-foreground pl-6">{t.spaces.classrooms.classroom()}</TableHead>
                <TableHead className="font-semibold text-muted-foreground">{t.spaces.classrooms.type()}</TableHead>
                <TableHead className="font-semibold text-muted-foreground">{t.spaces.classrooms.occupation()}</TableHead>
                <TableHead className="w-[150px] text-right font-semibold text-muted-foreground pr-6">{t.common.status()}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                return (
                  ...
                  >
                    <TableCell className="pl-6">
                      <div>
                        <div className="font-bold text-foreground">{item.classroom.name}</div>
                        <div className="font-mono text-xs font-medium text-muted-foreground">{item.classroom.code}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right pr-6">
                      {isAvailable
                        ? (
                            <Badge variant="outline" className="bg-green-500/10 text-green-700 hover:bg-green-500/20 border-green-200 dark:border-green-800 dark:text-green-400 rounded-lg">
                        ...
                        : item.classroom.status !== 'active'
                          ...
                          : (
                              <Badge variant="default" className="bg-blue-600 hover:bg-blue-700 rounded-lg">
