          ...
          <div className="space-y-4">
            <Alert variant={result.failed === 0 ? "default" : "destructive"}>
              ...
              <AlertDescription>
                ...
                {result.failed > 0 &&
                  ` - ${result.failed} ${t.timetables.status.error()}`}
              </AlertDescription>
            </Alert>
            <DialogFooter>
          ...
          <div className="space-y-6">
                ...
                <div className="flex items-center justify-center gap-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {allParsed.length} {t.timetables.preview.totalLines()}
                    </p>
                  </div>
                  <Button
                    variant="ghost"
            ...
            {preview.length > 0 && (
              <div className="rounded-xl border border-border/40 overflow-hidden">
                <div className="bg-muted/30 px-4 py-2 text-xs font-bold uppercase tracking-wider text-muted-foreground border-b border-border/40">
                  {t.timetables.preview.title()} ({countValid}{" "}
                  {t.timetables.preview.validLines()} /{allParsed.length})
                </div>
                    ...
                    <thead className="bg-muted/10 sticky top-0">
                      <tr>
                        <th className="px-3 py-2 text-left">
                          {t.timetables.columns.class()}
                        </th>
                        <th className="px-3 py-2 text-left">
                          {t.timetables.columns.subject()}
                        </th>
                        <th className="px-3 py-2 text-left">
                          {t.timetables.columns.teacher()}
                        </th>
                        <th className="px-3 py-2 text-left">
                          {t.timetables.columns.day()}
                        </th>
                        <th className="px-3 py-2 text-left">
                          {t.timetables.columns.time()}
                        </th>
                        <th className="px-3 py-2 text-left">
                          {t.timetables.columns.status()}
                        </th>
                      </tr>
                    </thead>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={countValid === 0 || importMutation.isPending}
              >
                {importMutation.isPending && (
                  <IconLoader2 className="mr-2 h-4 w-4 animate-spin" />
                )}
                {t.common.import()} ({countValid})
              </Button>
            </DialogFooter>
          </div>
        )}
