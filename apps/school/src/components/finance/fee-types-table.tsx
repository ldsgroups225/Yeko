        ...
        <Table>
          <TableHeader className="bg-muted/50">
            <TableRow className="hover:bg-transparent border-border/40">
              <TableHead className="font-semibold">{t.finance.feeTypes.code()}</TableHead>
              <TableHead className="font-semibold">{t.common.name()}</TableHead>
              <TableHead className="font-semibold">{t.finance.feeTypes.category()}</TableHead>
          ...
          <TableBody>
            <AnimatePresence>
              {feeTypes.map((feeType, index) => (
                ...
                >
                  <TableCell className="font-mono text-sm font-medium text-muted-foreground">{feeType.code}</TableCell>
                  <TableCell>
                    <div>
                      <div className="font-bold text-foreground">{feeType.name}</div>
                      <div className="flex gap-1 mt-1.5">
                        {feeType.isMandatory && (
                          <Badge variant="outline" className="text-[10px] uppercase font-bold tracking-wider bg-red-500/5 text-red-600 border-red-200 dark:border-red-900/30">
                            Obligatoire
                          </Badge>
                        )}
                        {feeType.isRecurring && (
                          <Badge variant="outline" className="text-[10px] uppercase font-bold tracking-wider bg-blue-500/5 text-blue-600 border-blue-200 dark:border-blue-900/30">
                            Récurrent
                          </Badge>
                        )}
                      </div>
                  ...
                  <TableCell>
                    <Badge variant={feeType.status === 'active' ? 'default' : 'secondary'} className="rounded-md">
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="text-xs font-mono text-muted-foreground mt-0.5">{feeType.code}</div>
                </div>
                <Badge variant={feeType.status === 'active' ? 'default' : 'secondary'} className="rounded-md">
                  {feeType.status === 'active' ? t.common.active() : t.common.inactive()}
                </Badge>
