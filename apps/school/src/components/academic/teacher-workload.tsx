...
export function TeacherWorkload() {
  ...
  const teacherWorkloads = teachers.map((teacher) => {
    const assignments = classSubjectsData?.filter(cs => cs.teacher?.id === teacher.id) || []
    const totalHours = assignments.reduce((sum: number, cs) => sum + (cs.classSubject.hoursPerWeek || 0), 0)
  ...
  return (
    <div className="space-y-6" role="region" aria-label={t.workload.ariaLabel()}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3" role="list" aria-label={t.teachers.list()}>
        {teacherWorkloads.map(({ teacher, totalHours, classCount, isOverloaded }, index) => (
          <motion.div
            key={teacher.id}
                ...
                <CardTitle className="flex items-center justify-between text-base">
                  ...
                  {isOverloaded
                    ...
                    : totalHours > 0
                      ...
                      : (
                          <Badge variant="outline" className="text-muted-foreground border-border/40">{t.workload.notAssigned()}</Badge>
                        )}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
