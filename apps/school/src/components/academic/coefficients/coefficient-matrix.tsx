...
import { schoolCoefficientsKeys, schoolCoefficientsOptions } from '@/lib/queries/school-coefficients'
import { bulkUpdateSchoolCoefficients, deleteCoefficientOverride } from '@/school/functions/school-coefficients'
...
export function CoefficientMatrix({ schoolYearTemplateId, seriesId }: CoefficientMatrixProps) {
  ...
  const bulkUpdateMutation = useMutation({
    mutationFn: (updates: { coefficientTemplateId: string, weightOverride: number }[]) =>
      bulkUpdateSchoolCoefficients({ data: { updates } }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: schoolCoefficientsKeys.all })
      toast.success('Coefficients updated successfully')
      setEditedCells(new Map())
    },
    onError: (err: Error) => {
      toast.error(err.message || 'Failed to update coefficients')
    },
  })

  const deleteOverrideMutation = useMutation({
    mutationFn: (overrideId: string) => deleteCoefficientOverride({ data: overrideId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: schoolCoefficientsKeys.all })
      toast.success('Reset to template default')
    },
    onError: (err: Error) => {
      toast.error(err.message || 'Failed to reset coefficient')
    },
  })
  ...
  const handleSaveChanges = () => {
    ...
    const updates = Array.from(editedCells.entries()).map(([templateId, weight]) => ({
      ...
      <Card className="border-destructive">
        <CardHeader>
          <CardTitle className="text-destructive flex items-center gap-2">
            ...
            Error Loading Coefficient Matrix
          </CardTitle>
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Coefficient Matrix</CardTitle>
          <CardDescription>
            Edit coefficients for each subject and grade combination. Override values are highlighted.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {grades.map(grade => (
                    <TableHead key={grade.id} className="text-center min-w-[100px]">
                      {grade.name}
                ...
                {subjects.map(subject => (
                  <TableRow key={subject.id}>
                    ...
                    {grades.map((grade) => {
                      ...
                      if (!cell) {
                        return (
                          <TableCell key={grade.id} className="text-center text-muted-foreground">
                            —
                          </TableCell>
                        )
                      }
                      ...
                      return (
                        <TableCell key={grade.id} className="p-2">
                          <CoefficientCell
                            ...
                            onEdit={value => handleCellEdit(cell.templateId, value)}
                            onReset={() => cell.overrideId && handleResetCell(cell.overrideId)}
                          />
                        </TableCell>
                      )
                    })}
          ...
          {subjects.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">Configure subjects first to see coefficients</p>
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent className="pt-6">
          <div className="flex flex-wrap gap-6 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span>Override (custom value)</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded border" />
              <span>Template (default value)</span>
            </div>
            <div className="flex items-center gap-2">
