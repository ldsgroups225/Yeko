          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Badge variant="secondary">
                {preview.length} {t('hr.users.totalRows')}
              </Badge>
              <Badge variant="default">
                {preview.filter((r) => !r.error).length} {t('hr.users.validRows')}
              </Badge>
              {preview.filter((r) => r.error).length > 0 && (
                <Badge variant="destructive">
                  {preview.filter((r) => r.error).length} {t('hr.users.invalidRows')}
                </Badge>
              )}
            </div>
          </div>
        ...
        <div className="rounded-lg border bg-card p-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span>
                {results.success} {t('hr.users.usersImported')}
              </span>
            </div>
            {results.failed > 0 && (
              <div className="flex items-center gap-2">
                ...
                <span>
                  {results.failed} {t('hr.users.usersFailed')}
                </span>
              </div>
            )}
          </div>
