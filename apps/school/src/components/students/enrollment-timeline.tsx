...
export function EnrollmentTimeline({ enrollments }: EnrollmentTimelineProps) {
  ...
  const sortedEnrollments = [...enrollments].sort(
    (a, b) => new Date(b.enrollment.enrollmentDate).getTime() - new Date(a.enrollment.enrollmentDate).getTime(),
  )
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    {item.schoolYear && (
                      <p className="text-sm text-muted-foreground">{item.schoolYear.name}</p>
                    )}
                  </div>
