    ...
    >
          ...
          <div className="flex flex-col items-center gap-6 md:flex-row">
            ...
            <div className="text-center md:text-left space-y-2">
              <h1 className="text-4xl font-bold tracking-tight text-foreground">
                {student.lastName}
                {' '}
                <span className="font-light text-muted-foreground">{student.firstName}</span>
              </h1>
              <div className="flex flex-wrap items-center justify-center gap-3 md:justify-start">
                <Badge variant="outline" className="text-sm px-3 py-1 font-mono uppercase tracking-widest bg-white/50 dark:bg-black/20">
                  {student.matricule}
                </Badge>
                <Badge className={`${statusColors[student.status as keyof typeof statusColors]} border-0 px-3 py-1 text-sm shadow-none`}>
                  {{
      ...
      <Tabs defaultValue="info" className="w-full">
        <TabsList className="mb-8 p-1 h-auto bg-white/40 dark:bg-black/40 backdrop-blur-md rounded-full border border-white/20 dark:border-white/10 w-full md:w-auto inline-flex justify-start">
          <TabsTrigger value="info" className="rounded-full px-6 py-2.5 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
            {t.students.personalInfo()}
          </TabsTrigger>
          <TabsTrigger value="parents" className="rounded-full px-6 py-2.5 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
            {t.students.parents()}
          </TabsTrigger>
          <TabsTrigger value="enrollments" className="rounded-full px-6 py-2.5 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all">
                  ...
                  {currentClass?.gradeName && currentClass?.section
                    ...
                    : (
                        <div className="flex flex-col items-center justify-center h-32 text-center text-muted-foreground">
                          ...
                          <Button variant="link" onClick={() => setEnrollmentDialogOpen(true)}>
                      ...
                      {parents.map((item, idx: number) => (
                        ...
                        >
                          ...
                          <div className="flex flex-col gap-2">
                            {item.canPickup && <Badge variant="outline" className="justify-center data-[state=active]:bg-green-100">{t.students.canPickup()}</Badge>}
                            {item.receiveNotifications && <Badge variant="outline" className="justify-center text-xs">{t.students.receivesNotifications()}</Badge>}
                          </div>
                        </motion.div>
                      ))}
                    </div>
...
function InfoRow({ label, value, highlight = false }: { label: string, value?: string | null, highlight?: boolean }) {
  return (
    <div className={`flex justify-between items-center py-2 border-b border-dashed border-border/50 last:border-0 ${highlight ? 'bg-primary/5 -mx-2 px-2 rounded-md' : ''}`}>
