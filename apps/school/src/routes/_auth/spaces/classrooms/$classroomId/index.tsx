    ...
    <div className="space-y-8 p-1">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-lg font-medium text-muted-foreground italic">{classroom.code}</p>
          </div>
        </div>
      ...
      <Tabs defaultValue="info" className="space-y-6">
        <TabsList className="bg-muted/20 backdrop-blur-md border border-border/40 p-1 rounded-2xl w-full sm:w-auto inline-flex gap-1">
          <TabsTrigger value="info" className="rounded-xl data-[state=active]:bg-card data-[state=active]:shadow-xl data-[state=active]:shadow-primary/5">{t.spaces.classroom.tabs.info()}</TabsTrigger>
          <TabsTrigger value="classes" className="rounded-xl data-[state=active]:bg-card data-[state=active]:shadow-xl data-[state=active]:shadow-primary/5">{t.spaces.classroom.tabs.classes()}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="info" className="space-y-6 mt-6">
          ...
          >
            <Card className="rounded-3xl border border-border/40 bg-card/40 backdrop-blur-xl shadow-sm hover:shadow-md transition-all duration-300">
              <CardHeader className="border-b border-border/40 bg-muted/5">
                <CardTitle className="text-lg font-bold uppercase tracking-wider text-muted-foreground">{t.spaces.classroom.details()}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 pt-6">
                <div className="flex items-center justify-between p-3 rounded-xl bg-muted/10 hover:bg-muted/20 transition-colors">
                  <span className="text-sm font-medium text-muted-foreground">{t.spaces.classroom.status()}</span>
                  <Badge variant={classroom.status === 'active' ? 'default' : 'secondary'} className="capitalize">
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-muted/10 hover:bg-muted/20 transition-colors">
                  <span className="text-sm font-medium text-muted-foreground">{t.spaces.classroom.type()}</span>
            ...
            <Card className="rounded-3xl border border-border/40 bg-card/40 backdrop-blur-xl shadow-sm hover:shadow-md transition-all duration-300">
              <CardHeader className="border-b border-border/40 bg-muted/5">
                <CardTitle className="text-lg font-bold uppercase tracking-wider text-muted-foreground">{t.spaces.classroom.location()}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 pt-6">
                <div className="flex items-center justify-between p-3 rounded-xl bg-muted/10 hover:bg-muted/20 transition-colors">
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-muted/10 hover:bg-muted/20 transition-colors">
                  <span className="text-sm font-medium text-muted-foreground">{t.spaces.classrooms.floor()}</span>
          ...
          {classroom.equipment && Object.keys(classroom.equipment).length > 0 && (
            ...
            >
              <Card className="rounded-3xl border border-border/40 bg-card/40 backdrop-blur-xl shadow-sm hover:shadow-md transition-all duration-300">
                <CardHeader className="border-b border-border/40 bg-muted/5">
                  <CardTitle className="text-lg font-bold uppercase tracking-wider text-muted-foreground">{t.spaces.classroom.equipment()}</CardTitle>
                </CardHeader>
                <CardContent className="pt-6">
                  <div className="flex flex-wrap gap-3">
                    {classroom.equipment.projector && <Badge variant="outline" className="px-3 py-1 rounded-lg text-sm bg-primary/5 border-primary/20 text-primary">{t.spaces.classroom.projector()}</Badge>}
                    {classroom.equipment.whiteboard && <Badge variant="outline" className="px-3 py-1 rounded-lg text-sm bg-primary/5 border-primary/20 text-primary">{t.spaces.classroom.whiteboard()}</Badge>}
                    {classroom.equipment.smartboard && <Badge variant="outline" className="px-3 py-1 rounded-lg text-sm bg-primary/5 border-primary/20 text-primary">{t.spaces.classroom.smartboard()}</Badge>}
                    {classroom.equipment.ac && <Badge variant="outline" className="px-3 py-1 rounded-lg text-sm bg-primary/5 border-primary/20 text-primary">{t.spaces.classroom.ac()}</Badge>}
                    {classroom.equipment.computers && (
                      <Badge variant="outline" className="px-3 py-1 rounded-lg text-sm bg-primary/5 border-primary/20 text-primary">
                        {classroom.equipment.computers}
                        {' '}
                    ...
                    {classroom.equipment.other?.map((item: string) => (
                      <Badge key={item} variant="outline" className="px-3 py-1 rounded-lg text-sm">{item}</Badge>
                    ))}
                  </div>
                </CardContent>
              </Card>
          ...
          {classroom.notes && (
            ...
            >
              <Card className="rounded-3xl border border-border/40 bg-card/40 backdrop-blur-xl shadow-sm hover:shadow-md transition-all duration-300">
                <CardHeader className="border-b border-border/40 bg-muted/5">
                  <CardTitle className="text-lg font-bold uppercase tracking-wider text-muted-foreground">{t.spaces.classroom.notes()}</CardTitle>
                </CardHeader>
                <CardContent className="pt-6">
                  <p className="text-sm font-medium text-muted-foreground leading-relaxed italic">{classroom.notes}</p>
                </CardContent>
              </Card>
            </motion.div>
          )}
        ...
        <TabsContent value="classes" className="mt-6">
          ...
          >
            <Card className="rounded-3xl border border-border/40 bg-card/40 backdrop-blur-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
              <CardHeader className="border-b border-border/40 bg-muted/5">
                <CardTitle className="text-lg font-bold uppercase tracking-wider text-muted-foreground">{t.spaces.classroom.assignedClasses()}</CardTitle>
              </CardHeader>
                        ...
                        {assignedClasses.map((cls, index) => (
                          ...
                          >
                            <span className="font-bold text-lg">
                              {cls.gradeName}
                              {' '}
                              <span className="font-normal text-muted-foreground">{cls.seriesName}</span>
                              {' '}
                              <span className="ml-2 px-2 py-0.5 rounded-md bg-muted/20 text-xs font-mono">{cls.section}</span>
                            </span>
                            <Button
                              variant="ghost"
                              size="sm"
