...
export const createPaymentPlanTemplateSchema = z.object({
  ...
  (data) => {
    const totalPercentage = data.schedule.reduce((sum, item) => sum + item.percentage, 0)
    return Math.abs(totalPercentage - 100) < 0.01
  },
  { message: 'Le total des pourcentages doit être égal à 100%', path: ['schedule'] },
)

export type CreatePaymentPlanTemplateInput = z.infer<typeof createPaymentPlanTemplateSchema>

// Update payment plan template schema
export const updatePaymentPlanTemplateSchema = createPaymentPlanTemplateSchema.partial().extend({
  id: z.string().min(1),
  status: z.enum(['active', 'inactive']).optional(),
})

export type UpdatePaymentPlanTemplateInput = z.infer<typeof updatePaymentPlanTemplateSchema>
...
export type CreatePaymentPlanFromTemplateInput = z.infer<typeof createPaymentPlanFromTemplateSchema>
