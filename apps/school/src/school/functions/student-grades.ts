...
export const getGradesByClass = authServerFn
  ...
  .handler(async ({ data, context }) => {
    if (!context?.school)
      return { success: false as const, error: 'SCHOOL_NOT_SELECTED' }
    ...
    if (R.isFailure(_result1))
      return { success: false as const, error: 'GRADES_FETCH_FAILED' }
...
export const getGrade = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(_result2))
      return { success: false as const, error: 'GRADE_FETCH_FAILED' }
...
export const createGrade = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(result))
      return { success: false as const, error: 'GRADE_CREATE_FAILED' }
...
export const updateGrade = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(_result3))
      return { success: false as const, error: 'GRADE_UPDATE_FAILED' }
    await createAuditLog({
      schoolId,
      userId,
      action: 'update',
...
export const deleteDraftGrades = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(_result4))
      return { success: false as const, error: 'DRAFT_DELETE_FAILED' }
    await createAuditLog({
      schoolId,
      userId,
      action: 'delete',
...
export const submitGradesForValidation = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(_result5))
      return { success: false as const, error: 'GRADES_SUBMIT_FAILED' }
    await createAuditLog({
      schoolId,
      userId,
      action: 'update',
...
export const validateGrades = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(_result6))
      return { success: false as const, error: 'GRADES_VALIDATE_FAILED' }
    await createAuditLog({
      schoolId,
      userId,
      action: 'update',
...
export const rejectGrades = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(_result7))
      return { success: false as const, error: 'GRADES_REJECT_FAILED' }
    await createAuditLog({
      schoolId,
      userId,
      action: 'update',
...
export const getPendingValidations = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(_result8))
      return { success: false as const, error: 'PENDING_VALIDATIONS_FETCH_FAILED' }
...
export const getGradeStatistics = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(_result9))
      return { success: false as const, error: 'GRADE_STATISTICS_FETCH_FAILED' }
...
export const getGradeValidationHistory = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(_result10))
      return { success: false as const, error: 'GRADE_HISTORY_FETCH_FAILED' }
...
export const getSubmittedGradeIds = authServerFn
  ...
  .handler(async ({ data, context }) => {
    ...
    if (R.isFailure(_result11))
      return { success: false as const, error: 'GRADE_IDS_FETCH_FAILED' }
