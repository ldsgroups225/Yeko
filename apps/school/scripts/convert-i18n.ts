...
function convertKeyToPath(key: string): string {
  // Convert 'common.save' to 'common.save'
  // Handle special cases like 'delete', 'class', 'export', 'import' which are reserved words
  const parts = key.split('.')
  const reservedWords = ['delete', 'class', 'export', 'import', 'default', 'new', 'return', 'switch', 'case', 'break', 'continue', 'for', 'while', 'if', 'else', 'try', 'catch', 'finally', 'throw', 'typeof', 'instanceof', 'void', 'null', 'undefined', 'true', 'false', 'in', 'of', 'with', 'function', 'var', 'let', 'const', 'enum', 'interface', 'type', 'namespace', 'module', 'declare', 'abstract', 'as', 'async', 'await', 'constructor', 'get', 'set', 'static', 'public', 'private', 'protected', 'readonly', 'super', 'this', 'yield']

  return parts.map((part) => {
    // If part is a reserved word or contains special characters, use bracket notation
    if (reservedWords.includes(part) || !/^[a-z_$][\w$]*$/i.test(part)) {
      return `['${part}']`
    }
    return `.${part}`
  }).join('').slice(1) // Remove leading dot
}
