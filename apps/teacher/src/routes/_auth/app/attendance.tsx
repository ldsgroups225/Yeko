...
function AttendancePage() {
  ...
  const { data: counts } = useQuery({
    ...
    queryFn: async (): Promise<CountsResponse> => {
      ...
      return {
        ...
        present: rosterData.roster.filter(s => s.attendance?.status === 'present').length,
        absent: rosterData.roster.filter(s => s.attendance?.status === 'absent').length,
        late: rosterData.roster.filter(s => s.attendance?.status === 'late').length,
        excused: rosterData.roster.filter(s => s.attendance?.status === 'excused').length,
        notMarked: rosterData.roster.filter(s => !s.attendance).length,
      }
    },
    enabled: Boolean(selectedClassId && context?.schoolYearId && rosterData?.roster),
  })
  ...
  return (
    <div className="flex flex-col gap-4 p-4 pb-20">
      <h1 className="text-xl font-semibold">{t('attendance.title', 'Présence')}</h1>

      {/* Class and Date Selection */}
      <Card>
        <CardHeader>
          <CardTitle>{t('attendance.selectClass', 'Sélectionner la classe')}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                <option value="">{t('attendance.selectClass', 'Sélectionner une classe')}</option>
              </select>
            </div>
            <div>
              <label className="text-sm font-medium">
      ...
      {selectedClassId && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle>{t('attendance.studentList', 'Liste des élèves')}</CardTitle>
