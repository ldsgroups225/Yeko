  ...
  return (
    <div className="flex flex-col gap-4 p-4 pb-20">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {data?.classes?.map((cls: { id: string; name: string; grade null; studentName: string |Count: number; subjectCount: number }) => (
              ...
              <CardContent>
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <div className="flex items-center gap-1">
                    ...
                    <span>{cls.studentCount} {t('classes.students', 'élèves')}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <IconBook className="w-4 h-4" />
                    <span>{cls.subjectCount} {t('classes.subjects', 'matières')}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
